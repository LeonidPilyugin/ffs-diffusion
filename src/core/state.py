...
class State:
    ...
    def write_lammps(self, path: Path):
        with open(path, "wb") as f:
            ...
            f.write(bytes.from_hex("0001"))
            f.write(bytes.from_hex("0002"))
            ...
            for i in range(nchunk - 1):
                ...
                for j in range(max_lines):
                    ...
                    f.write(struct.pack("<d", self.mean_positions[index,0]))
                    f.write(struct.pack("<d", self.mean_positions[index,1]))
                    f.write(struct.pack("<d", self.mean_positions[index,2]))
                    index += 1
